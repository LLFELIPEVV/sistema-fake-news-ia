...
BEST_MODEL_PATH = os.path.join(MODEL_DIR, "lstm_best_model.keras")
BEST_SCORE_PATH = os.path.join(MODEL_DIR, "lstm_best_score.txt")
...
def build_lstm_model_sequential(
    vocab_size,
    embedding_dim=128,
    sequence_length=200,
    lstm_units=128,
    ...
    """Construye modelo Sequential con Embedding + LSTM + Dense mejorado."""
    model = Sequential(
        [
            # Embedding layer
            Embedding(
                ...
                mask_zero=True,  # Maneja padding automáticamente
                name="embedding",
            ),
            ...
            # Batch normalization después de LSTM
...
if __name__ == "__main__":
    ...
    # Construir modelo
    print("[INFO] Construyendo modelo LSTM Sequential...")
    model = build_lstm_model_sequential(
    ...
    EPOCHS = 15  # Incrementado para mejor convergencia
    ...
    plot_training_history(
        history,
        filename="lstm_training_history.png",
    ...
    plot_confusion_matrix(
        y_valid,
        y_valid_pred,
        title="Matriz de confusión - Validación (LSTM)",
        filename="lstm_confusion_valid.png",
    )

    plot_confusion_matrix(
        y_test,
        y_test_pred,
        title="Matriz de confusión - Prueba (LSTM)",
        filename="lstm_confusion_test.png",
    )
