...
BEST_MODEL_PATH = os.path.join(MODEL_DIR, "bilstm_best_model.keras")
BEST_SCORE_PATH = os.path.join(MODEL_DIR, "bilstm_best_score.txt")
...
def build_bilstm_model_sequential(
    vocab_size,
    embedding_dim=128,
    sequence_length=200,
    lstm_units=128,
...
if __name__ == "__main__":
    ...
    print("[INFO] Construyendo modelo BiLSTM Sequential...")
    model = build_bilstm_model_sequential(
    ...
    plot_training_history(
        history,
        filename="bilstm_training_history.png",
    ...
    plot_confusion_matrix(
        y_valid,
        y_valid_pred,
        title="Matriz de confusión - Validación (BiLSTM)",
        filename="bilstm_confusion_valid.png",
    )
    plot_confusion_matrix(
        y_test,
        y_test_pred,
        title="Matriz de confusión - Prueba (BiLSTM)",
        filename="bilstm_confusion_test.png",
    )
    plot_metrics(
        y_valid,
        y_valid_pred,
        dataset_name="Validación (BiLSTM)",
        filename="bilstm_metrics_valid.png",
    )
    plot_metrics(
        y_test,
        y_test_pred,
        dataset_name="Prueba (BiLSTM)",
        filename="bilstm_metrics_test.png",
    )
    ...
    ax.set_title("Comparación de métricas entre Validación y Prueba (BiLSTM)")
    ax.set_ylabel("Valor")
    ax.set_ylim(0, 1)
    ...
    save_figure(fig, "bilstm_comparison_valid_test.png")
    plt.close(fig)
